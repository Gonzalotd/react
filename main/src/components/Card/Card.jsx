...
import ProductModal from '../../components/ProductsModal/ProductModal';
...
export default function Card({ searchTerm }) { 
  ...
  const [ setLoginRedirect] = useState('');
  
  useEffect(() => {
    if (searchTerm !== undefined && handleSearch) {
  ...
  const handleAddToCart = (product) => {
    ...
    console.log('➕ Agregando al carrito (Redux):', product);
  ...
  return (
    <div className="cards-section">
      ...
      <div className="cards">
        {(searchTerm ? filteredData : data).map((dataCard) => (
            ...
            {user && (
              <>
                <button
                  ...
                  onClick={() => handleDetailProduct(dataCard)}
                >
                  Mas Detalles
                </button>                    
                <button
                  className='cards-container__btn'
                  onClick={() => handleAddToCart(dataCard)}
                >
                  Añadir al Carrito
                </button>
              </>
